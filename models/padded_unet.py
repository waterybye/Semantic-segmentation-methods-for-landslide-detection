...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decode
        decode_block4 = self.soft_concat(bottleneck, encode_block4, 1)
        cat_layer3 = self.conv_decode4(decode_block4)
        decode_block3 = self.soft_concat(cat_layer3, encode_block3, 1)
        cat_layer2 = self.conv_decode3(decode_block3)
        decode_block2 = self.soft_concat(cat_layer2, encode_block2, 1)
        cat_layer1 = self.conv_decode2(decode_block2)
        decode_block1 = self.soft_concat(cat_layer1, encode_block1, 1)
    ...
    @staticmethod
    def soft_concat(upsampled: torch.Tensor, bypass: torch.Tensor, dim: int = 1):
        pad = bypass.shape[-1] - upsampled.shape[-1]
        left_pad, right_pad = pad // 2, pad - (pad // 2)
        upsampled = F.pad(upsampled, pad=[left_pad, right_pad, left_pad, right_pad], mode='replicate')
        return torch.cat((upsampled, bypass), dim)
