...
class ResNetModels(object):
    ...
    def __init__(self, configer):
        ...
        self.pretrained = './pretrained_models/3x3resnet50-imagenet.pth'

    def self_pretrained_resnet101(self):
        self_pretrained = 'data/models/resnet-kv/resnet101_1e-05_1.0-1.0-1.0_100_030.pth'
        model: nn.Module = torchvision.models.resnet101(num_classes=43)
        model.load_state_dict(torch.load(self_pretrained)['model_state_dict'])
    ...
    def resnet50(self, **kwargs):
        ...
        model = ResNet(Bottleneck, [3, 4, 6, 3], deep_base=False,
                       norm_type="sync_batchnorm", **kwargs)  # self.configer.get('network', 'norm_type'), **kwargs)
        model = ModuleHelper.load_model(model,
                                        pretrained=self.pretrained)  # self.configer.get('network', 'pretrained'))
        return model
