...
class NormalResnetBackbone(nn.Module):
    def __init__(self, orig_resnet):
        ...
        if hasattr(orig_resnet, 'prefix'):
            self.prefix = orig_resnet.prefix
        else:
            self.prefix = nn.Sequential(orig_resnet.conv1, orig_resnet.bn1, orig_resnet.relu)
...
class ResNetBackbone(object):
    ...
    def __call__(self):
        arch = self.configer
        multi_grid = [1, 2, 1]  # self.configer.get('network.multi_grid', default=None)
        ...
        elif arch == 'self_pretrained_resnet101':
            orig_resnet = self.resnet_models.self_pretrained_resnet101()
            arch_net = NormalResnetBackbone(orig_resnet)
        else:
